...
const WORKERS_SET = "timeflow:workers";
const HEARTBEAT_KEY_PREFIX = "timeflow:worker:";
...
opsRouter.get("/realtime", async (_req, res) => {
    ...
    const consumers = mapXinfoRows(consumersRows).map((c) => ({
        name: String(c.name ?? ""),
    ...
    ? pendingDetails.slice(0, PENDING_SAMPLE_COUNT).map((row: any) => {
