...
featuresRouter.get("/daily", async (req: AuthedRequest, res, next) => {
  try {
    ...
    const now = new Date();
    const from = q.from ? utcDayStart(new Date(q.from)) : utcDayStart(new Date(now.getTime() - 6 * 86400000));
...
featuresRouter.get("/export.csv", async (req: AuthedRequest, res, next) => {
    try {
        ...
        const now = new Date();
        const from = q.from ? utcDayStart(new Date(q.from)) : utcDayStart(new Date(now.getTime() - 29 * 86400000));
        const to = q.to ? utcDayStart(new Date(q.to)) : utcDayStart(now);
